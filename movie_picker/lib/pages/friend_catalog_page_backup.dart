...
class _FriendCatalogPageState extends State<FriendCatalogPage>
  ...
  Future<void> _fetchFriendMovies() async {
    ...
    // Gather all movie IDs from friend's data
    final allMovieIds = <int>{};
    allMovieIds.addAll(_friendData!.watchedMovieIds);
    allMovieIds.addAll(_friendData!.bookmarkedMovieIds);

    // Get friend's ratings
    _friendRatings = _friendData!.movieRatings;

    debugPrint('🔄 [FriendCatalog] Starting to fetch friend\'s movies...');
    debugPrint('   Total movie IDs to check: ${allMovieIds.length}');
    ...
    final missingIds = allMovieIds.difference(cachedIds);

    if (missingIds.isNotEmpty) {
      debugPrint('🔄 [FriendCatalog] Fetching ${missingIds.length} missing movies from TMDB...');
      
      // PARALLEL FETCHING: Fetch all missing movies simultaneously for better performance
      ...
      for (int i = 0; i < results.length; i++) {
        ...
        if (movie != null) {
          ...
          debugPrint('❌ [FriendCatalog] Failed to fetch movie ${missingIds.elementAt(i)}');
          failureCount++;
        }
      }
      ...
      debugPrint('✅ [FriendCatalog] All movies already cached!');
    }

    // Get updated cache and filter friend's movies
    final updatedCachedMovies = await widget.movieService.filterCachedMovies();
    
    _friendWatchedMovies = updatedCachedMovies
        .where((movie) => _friendData!.watchedMovieIds.contains(movie.id))
        .toList();
    
    _friendBookmarkedMovies = updatedCachedMovies
        .where((movie) => _friendData!.bookmarkedMovieIds.contains(movie.id))
        .toList();

    debugPrint('✅ [FriendCatalog] Final catalog loaded:');
    debugPrint('   Watched movies: ${_friendWatchedMovies.length} / ${_friendData!.watchedMovieIds.length}');
    debugPrint('   Bookmarked movies: ${_friendBookmarkedMovies.length} / ${_friendData!.bookmarkedMovieIds.length}');
    debugPrint('   Total ratings: ${_friendRatings.length}');
    
    // Check for any missing movies that couldn't be fetched
    final foundWatchedIds = _friendWatchedMovies.map((m) => m.id).toSet();
    final foundBookmarkedIds = _friendBookmarkedMovies.map((m) => m.id).toSet();
    
    final missingWatched = _friendData!.watchedMovieIds.difference(foundWatchedIds);
    final missingBookmarked = _friendData!.bookmarkedMovieIds.difference(foundBookmarkedIds);
    
    if (missingWatched.isNotEmpty || missingBookmarked.isNotEmpty) {
      debugPrint('⚠️ [FriendCatalog] Some movies could not be loaded:');
      if (missingWatched.isNotEmpty) {
        debugPrint('   Missing watched movies: ${missingWatched.length}');
      }
      if (missingBookmarked.isNotEmpty) {
        debugPrint('   Missing bookmarked movies: ${missingBookmarked.length}');
      }
    }
  }
