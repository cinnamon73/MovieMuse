...
class _HomeScreenState extends State<HomeScreen>
    ...
    try {
      ...
      if (currentUser != null) {
        ...
        if (totalInteractions >= 5) {
          ...
          finalMovies = await widget.recommendationService
          ...
          // Use TMDB's own sorting for new users
          finalMovies = widget.movieService.sortMoviesByQuality(filtered);
        }
      } else {
        finalMovies = widget.movieService.sortMoviesByQuality(filtered);
      }
      ...
      if (finalMovies.isEmpty && 
        ...
        finalMovies = await widget.movieService.getPopularMovies(excludeIds: excludeIds);
      }
  ...
  Future<void> _maybePreloadMoreInBackground() async {
    ...
    // If we have fewer than 20 movies in the filtered queue, preload more
  ...
  void _onSwipeLeft() {
    if (trendingCardQueue.isNotEmpty) {
      ...
      debugPrint('👈 [Trending] Swiping LEFT on: ${movie.title} (ID: ${movie.id})');
      
      // IMMEDIATE state update - this must happen synchronously
      setState(() {
        notInterestedMovieIds.add(movie.id);
        trendingCardQueue.removeLast();
        debugPrint('👈 [Trending] Movie removed from queue. Queue size now: ${trendingCardQueue.length}');
        debugPrint('👈 [Trending] Added to skipped list. Total skipped: ${notInterestedMovieIds.length}');
      });
      ...
      AnalyticsService().trackMovieSwipe(movie, 'dislike');
  ...
  void _onSwipeRight() {
    if (trendingCardQueue.isNotEmpty) {
      final movie = trendingCardQueue.last;
      debugPrint('👉 [Trending] Swiping RIGHT on: ${movie.title} (ID: ${movie.id})');
      ...
      setState(() {
        ...
        trendingCardQueue.removeLast();
        debugPrint('👉 [Trending] Movie removed from queue. Queue size now: ${trendingCardQueue.length}');
        debugPrint('👉 [Trending] Added to watched list. Total watched: ${watchedMovieIds.length}');
      });
      ...
      AnalyticsService().trackMovieSwipe(movie, 'like');
  ...
  void _onSwipeUp() async {
    if (trendingCardQueue.isNotEmpty) {
      ...
      // Track analytics for movie details view
      AnalyticsService().trackMovieSwipe(movie, 'details');
  ...
  void _onSwipeDown() {
    if (trendingCardQueue.isNotEmpty) {
      ...
      // Track analytics
      AnalyticsService().trackMovieSwipe(movie, 'bookmark');
  ...
  void _updateMovieRating(int movieId, double rating) {
    ...
    AnalyticsService().trackEngagement('movie_rated', {
      'movie_id': movieId,
      'rating': rating,
  ...
  void _ensureCardQueueHasCards() {
    setState(() {
      // Get available movies that haven't been interacted with AT ALL
      final availableMovies =
          filteredQueue.where((movie) {
            ...
            if (isInTrending || isInForYou) {
              debugPrint('🚫 Excluding movie already in queue: ${movie.title}');
              return false;
            }
            
            // SECOND: BLACKLIST ALL INTERACTED MOVIES - once you interact, it's gone forever
            if (isWatched) {
              debugPrint('🚫 BLACKLISTED watched movie: ${movie.title}');
              return false;
            }
            if (isSkipped) {
              debugPrint('🚫 BLACKLISTED skipped movie: ${movie.title}');
              return false;
            }
            if (isBookmarked) {
              debugPrint('🚫 BLACKLISTED bookmarked movie: ${movie.title}');
              return false;
            }
            
            // THIRD: Only show movies with NO interactions
            debugPrint('✅ Including fresh movie in queue: ${movie.title}');
            return true;
          }).toList();

      // Add movies to fill the queue up to 10 cards
      final neededCards = 10 - trendingCardQueue.length;
      if (neededCards > 0 && availableMovies.isNotEmpty) {
        final cardsToAdd = availableMovies.take(neededCards).toList();
        debugPrint('🎬 Adding ${cardsToAdd.length} cards to trending queue');
        trendingCardQueue.insertAll(
          0,
          cardsToAdd,
        ); // Insert at the beginning (bottom of stack)
      } else if (neededCards > 0) {
        debugPrint('⚠️ No available movies to add to queue. Watched: ${watchedMovieIds.length}, Skipped: ${notInterestedMovieIds.length}, Bookmarked: ${bookmarkedMovieIds.length}, Total filtered: ${filteredQueue.length}');
      }
    });
  }
                ...
                tabs: [
                  ...
                  Tab(
                        child: Row(
                          ...
                          children: [
                            Icon(
                              isAnonymousUser ? Icons.lock : Icons.psychology,
                              size: 18,
                              color: isAnonymousUser ? Colors.white.withValues(alpha: 0.4) : null,
                            ),
                            const SizedBox(width: 6),
                            Text(
                      'For You',
                        ...
                        builder: (context) => FilterDialog(
                              ...
                              onApply: (genres, language, timePeriod, platform) async {
                                setState(() {
                                  ...
                                  // Note: Platform filter not used in backup
                                });
        ...
        child: Builder(
          builder: (context) {
            if (currentView == HomePageView.watched) {
              ...
              if (!_watchedCacheLoaded) {
                widget.movieService.filterCachedMovies().then((allCachedMovies) async {
                  if (mounted) {
                    await _updateWatchedMoviesCache(allCachedMovies);
                    setState(() {
                      _watchedCacheLoaded = true;
                    });
                  }
                });
              ...
              debugPrint('📊 [Watched] Using cached movies to rate: ${_cachedMoviesToRate.length}');
              debugPrint('📊 [Watched] Using cached rated movies: ${_cachedRatedMovies.length}');

                  return WatchedListPage(
                key: ValueKey('watched-list-${currentUserData?.userId ?? 'anon'}'),
                moviesToRate: _cachedMoviesToRate,
                ratedMovies: _cachedRatedMovies,
                      ...
                      await Navigator.push(
                        ...
                        AnimationUtils.createFadeSlideRoute(
                          MovieDetailsPage(
                            ...
                            isWatched: watchedMovieIds.contains(movie.id),
                            currentRating: movieRatings[movie.id] ?? 0.0,
                            showRatingSystem: true, // No rating system for trending/for you movies
                            ...
                            if (rating > 0) watchedMovieIds.add(movie.id); // Ensure rated movies are counted as watched
                            _lastWatchedCacheUpdate = 0;
                          });
                          if (currentUserData != null) {
                            if (rating > 0) {
                              currentUserData!.movieRatings[movie.id] = rating;
                            } else {
                              currentUserData!.movieRatings.remove(movie.id);
                            }
                            widget.userDataService.setMovieRatingWithUserData(currentUserData!, movie.id, rating);
                          }
                          widget.movieService.filterCachedMovies().then((allCachedMovies) {
                            if (mounted) {
                              _updateWatchedMoviesCache(allCachedMovies);
              ...
              return FutureBuilder<List<Movie>>(
                future: widget.movieService.filterCachedMovies(),
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    return const Center(child: CircularProgressIndicator());
                  }

                  final allCachedMovies = snapshot.data ?? [];
                  final bookmarkedMovies =
                      allCachedMovies
                          .where((m) => bookmarkedMovieIds.contains(m.id))
                          .toList();
                  ...
                  return BookmarkedListPage(
                    key: ValueKey('bookmarked-list-${currentUserData?.userId ?? ''}-$_userDataTimestamp'),
                    ...
                    onRatingChanged: (movieId, rating) {
                      _updateMovieRatingById(movieId, rating);
                    },
                    onMoveToWatched: (movieId) {
                      _markWatched(movieId); // This removes from bookmarks and adds to watched
                    },
                    onBackPressed: _showMainView,
                  );
                },
              ...
              return TabBarView(
                ...
                children: [
                  ...
                  isAnonymousUser 
                    ? _buildSignUpPromptTab() // Only show the friendly lock screen
                    : Center(
                    child: Container(
                      constraints: const BoxConstraints(maxWidth: 500),
                      padding: const EdgeInsets.symmetric(horizontal: 8),
                      child: isLoading
                              ? AnimatedLoadingIndicator(
                                ...
                                subMessage: 'Analyzing your movie preferences',
                                color: Colors.deepPurple,
                              )
                              : error != null
                              ? Center(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    const SizedBox(height: 16),
                                    ElevatedButton(
                                      onPressed: _initializeForYouQueue,
                                      child: Text('Retry', style: AppTypography.buttonText),
                                    ),
                                  ],
                                ),
                              )
                              : forYouCardQueue.isEmpty
                              ? isLoading
                                  ? const ShimmerCardStack() // Show shimmer when loading
                                  : Center(
                                child: Column(
                                    ...
                                    (index) {
                                      ...
                                      return Positioned.fill(
                                        child: MovieCard(
                                          ...
                                          recommendedBy: movieRecommenders[movie.id], // Pass recommender info
                                        ),
                                      );
                                    },
                                  ),
                                  if (forYouCardQueue.isNotEmpty)
                                    Positioned.fill(
                                      child: SwipeableMovieCard(
                                        ...
                                        isBookmarked: bookmarkedMovieIds
                                            .contains(forYouCardQueue.last.id),
                                        isWatched: watchedMovieIds.contains(
                                          forYouCardQueue.last.id,
                                        ),
                                        movieService: widget.movieService,
                                        rating:
                                            movieRatings[forYouCardQueue
                                                .last
                                                .id] ??
                                            0.0,
                                        recommendedBy: movieRecommenders[forYouCardQueue.last.id], // Pass recommender info
                                      ),
                                    ),
                                ],
                              ),
